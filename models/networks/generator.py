...
class GeneratorConfig:
    netG_scale_capacity: float = 1.0
    netG_num_base_resnet_layers: int = 2
    netG_use_noise: bool = True
    netG_resnet_ch: int = 256
...
class StyleGAN2ResnetGenerator(BaseNetwork):
    ...
    def __init__(self, opt):
        ...
        self.add_module("SpatialCodeModulation", GeneratorModulation(self.global_code_ch, opt.spatial_code_ch))
        ...
        for i in range(opt.netG_num_base_resnet_layers):
            ...
            new_layer = ResolutionPreservingResnetBlock(opt, in_channel, out_channel, self.global_code_ch)
        ...
        for j in range(num_upsamplings):
            ...
            new_layer = UpsamplingResnetBlock(
                in_channel, out_channel, self.global_code_ch, blur_kernel, opt.netG_use_noise
            )
            self.add_module(layer_name, new_layer)
            in_channel = out_channel

        last_layer = ToRGB(out_channel, self.global_code_ch, blur_kernel=blur_kernel)
